...
source("single_kernel_classification_train.R")
source("single_kernel_classification_test.R")
...
pathway <- "none" #replace with "hallmark" if you would like to restrict SVM to Hallmark genes only
...
for (cohort in cohorts) {
  ...
  for (replication in 1:100) {
    if (file.exists(sprintf("%s/%s/svm_pathway_%s_measure_AUROC_replication_%d_result.RData", result_path, cohort, pathway, replication)) == FALSE) {
      ...
      for (fold in 1:fold_count) {
        ...
        for (C in C_set) {
          ...
          state <- single_kernel_classification_train(K_train, y_train, parameters)
          prediction <- single_kernel_classification_test(K_test, state)
      ...
      save("result", file = sprintf("%s/%s/svm_pathway_%s_measure_AUROC_replication_%d_result.RData", result_path, cohort, pathway, replication))
    }
  }  
}
