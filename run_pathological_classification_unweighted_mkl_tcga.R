...
for (cohort in cohorts) {
  ...
  for (replication in 1:100) {
    if (file.exists(sprintf("%s/%s/umkl_pathway_%s_measure_AUROC_replication_%d_result.RData", result_path, cohort, pathway, replication)) == FALSE) {
      ...
      for (fold in 1:fold_count) {
        ...
        for (C in C_set) {
          ...
          state <- single_kernel_classification_train(apply(K_train, c(1, 2), mean), y_train, parameters)
          prediction <- single_kernel_classification_test(apply(K_test, c(1, 2), mean), state)
      ...
      save("result", file = sprintf("%s/%s/umkl_pathway_%s_measure_AUROC_replication_%d_result.RData", result_path, cohort, pathway, replication))
    } 
  }
}
